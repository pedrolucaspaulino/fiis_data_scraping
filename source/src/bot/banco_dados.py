...
def insert_into_talela_infofii(nome, data_base, cotacao_data_base, valor_provento, data_pagamento, periodo_referencia, nome_tabela):
    try:
        criando_tabela_infofii(nome_tabela)
        cursor.execute("INSERT INTO OR NOT info_fii2 VALUES (?,?,?,?,?,?)", (nome, data_base, cotacao_data_base, valor_provento, data_pagamento, periodo_referencia))
        conexao.commit()
    except:
